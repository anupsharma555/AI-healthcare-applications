#Import necessary libraries to visualize DICOM slices and save as .png files.
#%%
import pydicom
import numpy as np
...
# Summary:
# This script loads a DICOM volume from a folder, sorts the slices using their spatial position,
# and stacks them into a 3D NumPy array. It then extracts and visualizes the middle slice from each
# of the three cardinal planes (axial, sagittal, and coronal), saving each as a PNG image. The script
# uses pydicom for reading DICOM files, NumPy for array manipulation, Pillow for image saving, and
# matplotlib for visualization. It also prints out metadata such as modality, pixel spacing, and slice
# thickness, and adjusts the aspect ratio for non-primary planes to account for anisotropic voxel sizes.